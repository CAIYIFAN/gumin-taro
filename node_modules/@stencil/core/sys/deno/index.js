...
function globToRegExp(e, {extended: t = !0, globstar: r = !0, os: n = NATIVE_OS} = {}) {
 if ("" == e) return /(?!)/;
 const o = "windows" == n ? "(?:\\\\|/)+" : "/+", a = "windows" == n ? "(?:\\\\|/)*" : "/*", i = "windows" == n ? [ "\\", "/" ] : [ "/" ], s = "windows" == n ? "(?:[^\\\\/]*(?:\\\\|/|$)+)*" : "(?:[^/]*(?:/|$)+)*", l = "windows" == n ? "[^\\\\/]*" : "[^/]*", c = "windows" == n ? "`" : "\\";
 let h = e.length;
 for (;h > 1 && i.includes(e[h - 1]); h--) ;
 e = e.slice(0, h);
 let u = "";
 for (let n = 0; n < e.length; ) {
  ...
  const d = [];
  let f = !1, m = !1, p = !1, g = n;
  for (;g < e.length && !i.includes(e[g]); g++) if (m) m = !1, h += (f ? rangeEscapeChars : regExpEscapeChars).includes(e[g]) ? "\\" + e[g] : e[g]; else if (e[g] != c) {
   if ("[" == e[g]) {
    if (!f) {
     f = !0, h += "[", "!" == e[g + 1] ? (g++, h += "^") : "^" == e[g + 1] && (g++, h += "\\^");
     continue;
    }
    if (":" == e[g + 1]) {
     let t = g + 1, r = "";
     for (;null != e[t + 1] && ":" != e[t + 1]; ) r += e[t + 1], t++;
     if (":" == e[t + 1] && "]" == e[t + 2]) {
      g = t + 2, "alnum" == r ? h += "\\dA-Za-z" : "alpha" == r ? h += "A-Za-z" : "ascii" == r ? h += "\0-" : "blank" == r ? h += "\t " : "cntrl" == r ? h += "\0-" : "digit" == r ? h += "\\d" : "graph" == r ? h += "!-~" : "lower" == r ? h += "a-z" : "print" == r ? h += " -~" : "punct" == r ? h += "!\"#$%&'()*+,\\-./:;<=>?@[\\\\\\]^_‘{|}~" : "space" == r ? h += "\\s\v" : "upper" == r ? h += "A-Z" : "word" == r ? h += "\\w" : "xdigit" == r && (h += "\\dA-Fa-f");
      continue;
     }
    }
   }
   if ("]" == e[g] && f) f = !1, h += "]"; else if (f) "\\" == e[g] ? h += "\\\\" : h += e[g]; else if (")" == e[g] && d.length > 0 && "BRACE" != d[d.length - 1]) {
    h += ")";
    const e = d.pop();
    "!" == e ? h += l : "@" != e && (h += e);
   } else if ("|" == e[g] && d.length > 0 && "BRACE" != d[d.length - 1]) h += "|"; else if ("+" == e[g] && t && "(" == e[g + 1]) g++, 
   d.push("+"), h += "(?:"; else if ("@" == e[g] && t && "(" == e[g + 1]) g++, d.push("@"), 
   h += "(?:"; else if ("?" != e[g]) if ("!" == e[g] && t && "(" == e[g + 1]) g++, 
   d.push("!"), h += "(?!"; else if ("{" != e[g]) if ("}" != e[g] || "BRACE" != d[d.length - 1]) if ("," != e[g] || "BRACE" != d[d.length - 1]) if ("*" != e[g]) h += regExpEscapeChars.includes(e[g]) ? "\\" + e[g] : e[g]; else if (t && "(" == e[g + 1]) g++, 
   d.push("*"), h += "(?:"; else {
    const t = e[g - 1];
    let n = 1;
    for (;"*" == e[g + 1]; ) g++, n++;
    const o = e[g + 1];
    r && 2 == n && [ ...i, void 0 ].includes(t) && [ ...i, void 0 ].includes(o) ? (h += s, 
    p = !0) : h += l;
   } else h += "|"; else d.pop(), h += ")"; else d.push("BRACE"), h += "(?:"; else t && "(" == e[g + 1] ? (g++, 
   d.push("?"), h += "(?:") : h += ".";
  } else m = !0;
  if (d.length > 0 || f || m) {
   h = "";
   for (const t of e.slice(n, g)) h += regExpEscapeChars.includes(t) ? "\\" + t : t, 
   p = !1;
  }
  for (u += h, p || (u += g < e.length ? o : a, p = !0); i.includes(e[g]); ) g++;
  if (!(g > n)) throw new Error("Assertion failure: i > j (potential infinite loop)");
  n = g;
 }
 return u = `^${u}$`, new RegExp(u);
}
...
async function _createWalkEntry(e) {
 e = normalize$2(e);
...
async function* walk(e, {maxDepth: t = 1 / 0, includeFiles: r = !0, includeDirs: n = !0, followSymlinks: o = !1, exts: a, match: i, skip: s} = {}) {
 if (!(t < 0) && (n && include(e, a, i, s) && (yield await _createWalkEntry(e)), 
 !(t < 1) && include(e, void 0, void 0, s))) for await (const l of Deno.readDir(e)) {
  if (l.isSymlink) {
     ...
     for (o = __asyncValues(async function* s(e, {root: t = Deno.cwd(), exclude: r = [], includeDirs: n = !0, extended: o = !1, globstar: a = !1} = {}) {
      async function* i(e, t) {
       if (e.isDirectory) if (".." != t) {
        ...
        try {
         if ((e => !h.some(t => !!e.match(t)))(t)) return yield await _createWalkEntry(t);
        } catch (e) {
      ...
      for (;u.length > 0 && !isGlob(u[0]); ) {
       const e = u.shift();
       assert(null != e), g = joinGlobs([ g, e ], s);
      }
      try {
       p = await _createWalkEntry(g);
      } catch (e) {
...
var _a, _b, _c, _a$1, _b$1, __asyncValues, __asyncValues$1;
...
const createTerminalLogger = e => {
 ...
 }, c = t => e.color(t, "red"), h = t => e.color(t, "yellow"), u = t => e.color(t, "cyan"), d = t => e.color(t, "bold"), f = t => e.color(t, "dim"), m = t => e.color(t, "bgRed"), p = e => LOG_LEVELS.indexOf(e) >= LOG_LEVELS.indexOf(t), g = (t, r, n) => {
  var o, a;
   ...
   r.forEach(r => {
    l = l.concat(((r, n) => {
     ...
     r.lines && r.lines.length && (prepareLines(r.lines).forEach(e => {
      ...
      let n = e.text;
      e.errorCharStart > -1 && (n = g(n, e.errorCharStart, e.errorLength)), t = f(t), 
...
const isWindows = "windows" == NATIVE_OS;
...
let enabled = !(null === (_b$1 = null === (_a$1 = globalThis.Deno) || void 0 === _a$1 ? void 0 : _a$1.noColor) || void 0 === _b$1 || _b$1);
