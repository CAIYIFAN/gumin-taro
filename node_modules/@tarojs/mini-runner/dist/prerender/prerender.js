...
function getAttrValue(value) {
    ...
    if (value === 'true' || value === 'false' || !shared_1.isString(value)) {
        return `"{{${value}}}"`;
    }
...
class Prerender {
    constructor(buildConfig, webpackConfig, stat) {
        ...
        this.renderToXML = (data) => {
            ...
            return `<${nodeName}${style ? ` style="${style}"` : ''}${klass ? ` class="${klass}"` : ''} ${this.buildAttributes(attrs)}>${children.map(this.renderToXML).join('')}</${nodeName}>`;
        };
    ...
    render() {
      ...
      `, this.outputPath);
                    ...
                    logHelper_1.printPrerenderFail('app');
                    console.error(error);
                }
                this.appLoaded = true;
    ...
    getRealPath(path, ext = '.js') {
        return path_1.join(this.outputPath, path + ext).replace(/\\/g, '\\\\');
    }
    buildSandbox() {
